...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('customer-analysis-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('customer-analysis-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('customer-analysis-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('customer-analysis-progress', JSON.stringify(this.progress));
        localStorage.setItem('customer-analysis-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>顧客分析アカデミー へようこそ</h2>
                    <p>営業の「なぜ売れた」「なぜ失注した」を科学し、データドリブンな営業組織を構築するための実践講座です。</p>
                </div>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('customer-analysis-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('customer-analysis-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('customer-analysis-progress');
            localStorage.removeItem('customer-analysis-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
